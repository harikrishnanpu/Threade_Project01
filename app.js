...
app.use((req, res, next) => {
  res.status(404).render('404',{ noHeader: true, noFooter: true, user: null,  noSidebar: true })
});

app.use((err, req, res, next) => {
  res.status(500).render('500',{ message: err.message, noHeader: true, noFooter: true, user:  req.user || req.admin, noSidebar: true})
});
